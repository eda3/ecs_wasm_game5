...
- [ ] レンダリング実装(Rust/Wasm): **Canvas** を使った基本的なカード描画 (`app/renderer.rs` の `render_game_rust`) - 描画ロジック本体は **未実装！** 🖌️
...
- [/] テスト(Rust): カード生成 (`systems/deal_system.rs`) のユニットテスト作成 - **`World` 実装済、テスト可能に！要修正・実行！** ➡️
- [/] テスト(Rust): カード移動ルール (`logic/rules.rs`) のユニットテスト作成 - **`World` 実装済、World 連携部分のテスト追加・実行！** ➡️
