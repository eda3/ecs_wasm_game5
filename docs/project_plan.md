...
## フェーズ 1: プロジェクトセットアップ & ECS基盤構築 🏗️ (**最重要課題あり！**)
...
- [ ] ECSコア実装: `World` 構造体の実装 (`world.rs`) - **基本的な構造体はあるが、主要メソッドが未実装！**
- [ ] ECSコア実装: `World` エンティティ作成・削除 - `create_entity`はあるが削除は未実装。
- [!] ECSコア実装: `World` コンポーネント追加・削除・取得 - **🚨 最重要課題！ `unimplemented!()` のまま！これが原因で他が動かない！**
...
- [!] ゲーム初期化: 初期カード配置 (ディール) システムの実装 (`deal_system.rs`) - **🚨 `World` 未実装のため未完了！** 🚧
- [x] レンダリング準備: `GameApp` (WASM側メイン構造体) の実装 (`lib.rs`) - 構造体はあるが `World` 未実装のため一部機能不全。
- [!] レンダリング準備: `GameApp` から World の状態を取得するメソッド実装 (`get_world_state_json`) - **🚨 `World` 未実装のため正しく動作しない可能性大！**
...
## フェーズ 3: ネットワーク実装 🌐🤝 (ほぼ完了！🎉 一部ブロッカーあり)
...
- [!] クライアント側ネットワーク処理: `apply_game_state()` で受信データ (`GameStateData`) を `World` に反映 (`lib.rs`) - **🚨 `World` 未実装のため要修正！** 🔧
...
## フェーズ 4: ソリティアのルールとインタラクション実装 🃏👆 (Canvasへ移行中！一部ブロッカーあり)

- [/] ルール実装(Rust): `StackType` ごとのカード移動可否判定ヘルパー関数 (`src/rules.rs`) - 基本ヘルパー実装済 ✅ **⚠️ 注意: `World` 未実装のため、World状態に依存する部分は未検証/未完了！** 🔧
...
- [/] ルール実装(Rust): ゲームクリア判定ロジック (`check_win_condition`) ✅ (`WinConditionSystem` は **🚨 `World` 未実装により動作しない**)
...
- [!] 状態更新と表示(JS): `apply_game_state` (Rust) 後、**Canvas描画関数** (`render_game_rust`) を呼び出して画面を更新 - **🚨 `apply_game_state` と `render_game_rust` が未完成なため未完了！**
...
## フェーズ 6: テストとデプロイ 🧪🚀 (一部開始、ブロッカーあり)

- [!] テスト(Rust): ECSコア (`World`, `Entity`, `Component`) のユニットテスト作成 - **🚨 `World` 未実装のためテスト不可！** (`entity.rs`, `component.rs` に一部テストあり)
- [!] テスト(Rust): カード生成 (`deal_system`) のユニットテスト作成 - **🚨 `World` 未実装のためテスト不可！** (`deal_system.rs` にテストあるが動作しない)
- [!] テスト(Rust): カード移動ルール のユニットテスト作成 - **🚨 `World` 未実装のため World 連携部分はテスト不可！** (`rules.rs` に基本テストあり)
